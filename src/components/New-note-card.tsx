...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps){
  ...
  function handleStartRecording(){
    ...
    if(!isSpeechRecognitionAPIAvailable){
      alert('Infelizmente seu navegador não suporta o reconhecimento por áudio :(');
      return;
    }
          ...
          <form className='flex-1 flex flex-col'>
            <div className='flex flex-1 flex-col gap-3 p-5'>
              ...
              {shouldShowOnboarding ? (
                ...
                <textarea onChange={handleContentChanged} autoFocus value={content} className='text-sm leading-6 text-slate-400 bg-transparent resize-none flex-1 outline-none' />
              )}
            </div>
